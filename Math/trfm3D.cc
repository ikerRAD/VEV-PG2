...
Vector3 Trfm3D::transformPoint(const Vector3 & P) const {
	Vector3 res;
	//Multiplicamos  el punto con la matríz
	res[0] = m_scl*(m_c1[0]*P[0] + m_c2[0]*P[1] + m_c3[0]*P[2]) + m_tr[0];
	res[1] = m_scl*(m_c1[1]*P[0] + m_c2[1]*P[1] + m_c3[1]*P[2]) + m_tr[1];
	res[2] = m_scl*(m_c1[2]*P[0] + m_c2[2]*P[1] + m_c3[2]*P[2]) + m_tr[2];
...
Vector3 Trfm3D::transformVector(const Vector3 & V) const {
	Vector3 res;
    //Se multiplica el vector con la matriz
    //Hay que tener en cuenta que en coor homogeneas, un vector tiene 0 como coord extra
	res[0] = m_scl*(m_c1[0]*V[0] + m_c2[0]*V[1] + m_c3[0]*V[2]);
	res[1] = m_scl*(m_c1[1]*V[0] + m_c2[1]*V[1] + m_c3[1]*V[2]);
	res[2] = m_scl*(m_c1[2]*V[0] + m_c2[2]*V[1] + m_c3[2]*V[2]);
...
// Rotate angle radians about an axis defined by vector and located at point
//

void Trfm3D::setRotAxis(const Vector3 & V, const Vector3 & P, float angle ) {
    //Estrablece la transformación
	setTrans(P);
	//Añade una rotación de angle grados sobre el eje V
	addRotVec(V,angle);
