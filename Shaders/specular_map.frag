...
void aporte_dir(in int i,in vec3 l,in vec3 n,in vec3 v,inout vec3 acum1, inout vec3 acum2, in vec4 texel){
	...
	if (factor1 > 0.0){
		...
		if (factor2 > 0.0){
			...
			acum2 = acum2 + factor2 * texel.rgb * theLights[i].specular;
		}
...
void aporte_pos(in int i,in vec3 l,in vec3 n,in vec3 v,inout vec3 acum1, inout vec3 acum2, in vec4 texel){
	...
	if (factor1 > 0.0){
		...
		if (factor2 > 0.0){
			factor2 = factor1 * pow(factor2, theMaterial.shininess);
			acum2 = acum2 + atenuacion * factor2 * texel.rgb * theLights[i].specular;
		}
	...
	if (cosOs > 0.0){//Evitamos cálculos
		if (cosOs >= theLights[i].cosCutOff){//si Os es menor o igual que Ou
			...
			if (factor1 > 0.0){
				...
				if (factor2 > 0.0){
					factor2 = factor1 * pow(factor2, theMaterial.shininess);
					acum2 = acum2 + spt * factor2 * texel.rgb * theLights[i].specular;
				}
