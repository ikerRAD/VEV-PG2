...
void PerspectiveCamera::updateProjection() {
	//Calculamos los parámetros necesarios para crear la proyección en perspectiva
	m_top = m_near * tan(m_fovy/2);
	m_bottom = -m_top;
	m_right = m_aspectRatio * m_top;
	m_left = -m_right;
    //Ponemos en marcha la perspectiva con los datos calculados
...
void  Camera::lookAt(const Vector3 & E,
	...
	m_E = E;
    //Vector D (Z) de la cámara
	m_D = (E - at).normalize();

	Vector3 aux = up.normalize();
	//Vector R (X) de la cámara
	m_R = aux.cross(m_D);
    //Vector U (Y) de la cámara
	m_U = m_D.cross(m_R);
	setViewTrfm();
}
...
int Camera::checkFrustum(const BBox *theBBox,
	...
	for(int i = 0; i < 6; i++){
		provisional = BBoxPlaneIntersect(theBBox, m_fPlanes [i]);

		if( provisional == IREJECT ){
			return 1;
		}else if (provisional == IINTERSECT){
			intersecta ++;
		}
	}
